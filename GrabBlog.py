...
for page in range(1,int(npages)+1):
    response = None
    response = urlopen(base_url+'/posts?page=' + str(page))
    if response == None:
        print(base_url + '/posts/' + postID + '擷取不能: 請檢察網站位置是否正確，是否可以直接連上或者重新執行一次(因為天空有時候怪怪的)')
    else:
        html = response.read().decode("utf8")
        soup = BeautifulSoup(html, "html.parser")
        anchors = soup.find_all('a',class_='btn-read btn btn-blue with-radius with-border')
        ...
        h = open(SaveAs + 'page' + str(page) + '.htm','w', encoding = 'utf8')
        err=h.write(html)
        err=h.close()

page_list1 = []
for i in page_list:
    if not i in page_list1:
        page_list1.append(i)
page_list = page_list1
print('共' + str(len(page_list)) + '個目標')

for n in range(0,len(page_list)):
    start = page_list[n].find('yam.com/posts/')
    albumID = page_list[n][start+13:len(page_list[n])]
    response = None
    response = urlopen(page_list[n])
    if response == None:
        print(base_url + '/posts/' + postID + '擷取不能: 請檢察網站位置是否正確，是否可以直接連上或者重新執行一次(因為天空有時候怪怪的)')
    else:
        html = response.read().decode("utf8")
        ...
        if(soup.find('h1')!= None):
            post_title = str(albumID) + '_' + soup.find('h1').text
